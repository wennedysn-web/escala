...
const App: React.FC = () => {
  ...
  const loadInitialData = async () => {
    ...
    setError(null);
    try {
      // Carregamento paralelo de todas as tabelas essenciais
      ...
      // Verifica erros em qualquer uma das requisições
      ...
      setError("Não foi possível carregar os dados do banco. Verifique as tabelas no Supabase.");
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[60] flex items-center justify-center">
            <div className="bg-white p-10 rounded-[40px] shadow-2xl flex flex-col items-center gap-6 border border-slate-100">
              ...
              <div className="text-center">
                <p className="font-black text-xl text-slate-900">Processando dados...</p>
                <p className="text-slate-500 text-sm mt-1 font-medium">Aguarde enquanto sincronizamos as informações.</p>
              </div>
            </div>
          </div>
        )}
...
const Login: React.FC<{ setSession: (s: any) => void }> = ({ setSession }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Aceita admin/tododia ou escala@gmail.com/escala@gmail.com ou escala@gmail.com/tododia
      ...
      <div className="w-full max-w-md bg-white p-10 md:p-12 rounded-[40px] shadow-2xl space-y-10 border border-white/20">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-2">
            <label className="text-[11px] font-black text-slate-800 uppercase tracking-widest pl-1">Usuário / E-mail</label>
            <input 
              type="text" 
              value={username} 
              onChange={e => setUsername(e.target.value)} 
              className="w-full px-6 py-4 bg-slate-50 border-2 border-slate-200 text-slate-900 font-bold rounded-[20px] outline-none focus:ring-4 focus:ring-blue-100 focus:border-blue-600 transition-all placeholder-slate-400" 
              placeholder="Digite seu usuário"
              required 
            />
          </div>
          <div className="space-y-2">
            <label className="text-[11px] font-black text-slate-800 uppercase tracking-widest pl-1">Senha</label>
            <input 
              type="password" 
              value={password} 
          ...
          {error && <p className="text-red-600 text-sm font-black text-center animate-shake bg-red-50 p-3 rounded-xl">{error}</p>}
  ...
  <div className="space-y-10">
    <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 bg-white p-10 rounded-[40px] shadow-sm border border-slate-200/50">
      <div>
        ...
        <p className="text-slate-600 font-bold mt-1">Gerencie as escalas de domingos e feriados.</p>
      </div>
...
const TabBtn: React.FC<{ active: boolean; onClick: () => void; label: string }> = ({ active, onClick, label }) => (
  <button onClick={onClick} className={`px-10 py-5 text-[11px] font-black uppercase tracking-widest transition-all rounded-2xl whitespace-nowrap ${active ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-600 hover:text-slate-900'}`}>
    {label}
  </button>
);
  ...
  return (
    <div className="space-y-10 animate-in fade-in">
      <div className="flex gap-4">
        <input value={name} onChange={e => setName(e.target.value)} placeholder="Ex: Porteiro..." className="flex-1 bg-slate-50 border border-slate-200 px-6 py-4 rounded-[20px] text-slate-900 font-bold outline-none focus:border-blue-600" />
  ...
  return (
    <div className="space-y-10 animate-in fade-in">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <input value={name} onChange={e => setName(e.target.value)} placeholder="Nome" className="md:col-span-2 bg-slate-50 border border-slate-200 px-6 py-4 rounded-[20px] text-slate-900 font-bold outline-none" />
        <select value={cat} onChange={e => setCat(e.target.value)} className="bg-slate-50 border border-slate-200 px-6 py-4 rounded-[20px] text-slate-900 font-bold outline-none">
      ...
      <label className="flex items-center gap-4 cursor-pointer">
        ...
        <span className="text-xs font-black text-slate-800 uppercase">Colaborador com Restrição (Nunca trabalha só)</span>
      </label>
        ...
        {employees.map((e: any) => (
          <div key={e.id} className="p-6 bg-slate-50 border border-slate-200 rounded-[28px] flex justify-between items-center">
            <div>
              ...
              <p className="text-[10px] text-slate-600 font-black uppercase tracking-wider">{categories.find((c: any) => c.id === e.categoryId)?.name}</p>
            </div>
    ...
    <div className="space-y-10 animate-in fade-in">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        <div className="space-y-6">
          <input value={name} onChange={e => setName(e.target.value)} placeholder="Nome do Posto/Ambiente" className="w-full bg-slate-50 border border-slate-200 px-6 py-4 rounded-[20px] text-slate-900 font-bold outline-none" />
          <div className="bg-slate-50 p-6 rounded-[32px] space-y-4 border border-slate-200">
            <p className="text-[10px] font-black text-slate-800 uppercase tracking-widest">Requisitos de Equipe:</p>
            {categories.map((c: any) => (
              <div key={c.id} className="flex items-center justify-between">
                <span className="text-sm font-bold text-slate-800">{c.name}</span>
                <input type="number" min="0" value={reqs[c.id] || 0} onChange={e => setReqs({...reqs, [c.id]: parseInt(e.target.value)||0})} className="w-16 py-2 bg-white border border-slate-300 rounded-xl text-center font-black text-slate-900 shadow-sm" />
              </div>
            ))}
          </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {Object.entries(env.requirements).map(([cid, qty]: any) => qty > 0 && (
                    <div key={cid} className="text-[9px] font-black bg-white border border-slate-200 px-3 py-1 rounded-lg text-slate-800 shadow-sm">
...
const DaySetup: React.FC<any> = ({ days, onAdd, onDel }) => {
  ...
  return (
    <div className="space-y-10 animate-in fade-in">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <input type="date" value={date} onChange={e => setDate(e.target.value)} className="bg-slate-50 border border-slate-200 px-6 py-4 rounded-[20px] text-slate-900 font-bold outline-none" />
        <select value={type} onChange={e => setType(e.target.value)} className="bg-slate-50 border border-slate-200 px-6 py-4 rounded-[20px] text-slate-900 font-bold outline-none">
          <option value="holiday">Feriado</option>
          <option value="sunday">Domingo</option>
        </select>
        <input value={name} onChange={e => setName(e.target.value)} placeholder="Identificação" className="bg-slate-50 border border-slate-200 px-6 py-4 rounded-[20px] text-slate-900 font-bold outline-none" />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {days.map((d: any) => (
          <div key={d.date} className="p-6 bg-slate-50 border border-slate-200 rounded-[28px] flex justify-between items-center shadow-sm">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-[10px] font-black text-blue-700 uppercase tracking-widest">{d.name}</p>
              </div>
            </div>
    ...
    <div className="space-y-10 animate-in fade-in">
      <header className="flex flex-col lg:flex-row justify-between items-center gap-8 bg-white p-10 rounded-[40px] shadow-sm border border-slate-200/50">
        <div className="flex items-center gap-6">
          <button onClick={() => onMonthChange(-1)} className="p-4 bg-slate-50 border border-slate-300 rounded-2xl hover:bg-blue-600 hover:text-white transition-all"><ChevronLeft/></button>
          <h2 className="text-3xl font-black text-slate-900 capitalize min-w-[200px] text-center">{monthLabel}</h2>
          <button onClick={() => onMonthChange(1)} className="p-4 bg-slate-50 border border-slate-300 rounded-2xl hover:bg-blue-600 hover:text-white transition-all"><ChevronRight/></button>
        </div>
      ...
      {error && (
        <div className="bg-red-50 p-6 rounded-[30px] border border-red-200 flex items-center gap-4 text-red-600 font-black">
      ...
      {displayDays.length === 0 ? (
        <div className="bg-white p-24 rounded-[60px] border-2 border-dashed border-slate-300 text-center space-y-4">
          ...
          <p className="text-slate-700 font-bold max-w-md mx-auto">Cadastre os Domingos e Feriados deste mês na aba de Configurações para gerar a escala.</p>
        </div>
      ) : (
              ...
              <div key={specialDay.date} className="bg-white rounded-[40px] p-8 border border-slate-200/50 shadow-sm flex flex-col hover:shadow-xl transition-all ring-offset-2 ring-blue-50 hover:ring-2">
                <div className="flex justify-between items-start mb-6">
                  <div className="flex flex-col">
                    ...
                    <span className="text-[10px] font-black text-slate-700 uppercase tracking-wider mt-1">{dateObj.toLocaleDateString('pt-BR', {weekday: 'long', timeZone: 'UTC'})}</span>
                  </div>
                ...
                <div className="space-y-3">
                  {dayEntries.length === 0 ? (
                    <p className="text-xs text-slate-500 italic font-bold">Escala não gerada para este dia.</p>
                  ) : (
                    dayEntries.map((e: any) => {
                      ...
                      return (
                        <div key={e.employeeId + e.environmentId} className="group relative bg-slate-50 p-4 rounded-[20px] border border-slate-300 hover:bg-blue-600 hover:border-blue-700 hover:text-white transition-all shadow-sm">
                          <p className="text-[12px] font-black text-slate-900 group-hover:text-white truncate pr-2">{emp?.name || 'Vazio'}</p>
                          <p className="text-[9px] font-black text-slate-600 group-hover:text-blue-100 mt-0.5 uppercase flex items-center gap-1 tracking-wider"><Building2 size={10}/> {env?.name}</p>
    ...
    <div className="space-y-10 animate-in fade-in">
      <div className="flex flex-col md:flex-row justify-between items-center gap-6">
        ...
        <div className="relative w-full md:w-80">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Localizar..." className="w-full pl-14 pr-6 py-4 bg-white border-2 border-slate-300 rounded-[24px] outline-none shadow-sm focus:border-blue-600 transition-all text-slate-900 font-bold" />
        </div>
      </div>
        ...
        <table className="w-full text-left">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-200">
              <th className="p-8 text-[11px] font-black text-slate-800 uppercase tracking-widest">Colaborador</th>
              <th className="p-8 text-[11px] font-black text-slate-800 uppercase tracking-widest">Categoria</th>
              <th className="p-8 text-[11px] font-black text-slate-800 uppercase tracking-widest">Status</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100">
            {filtered.map(e => (
              <tr key={e.id} className="hover:bg-slate-50/50 transition-colors">
                ...
                <td className="p-8">
                  <span className="px-5 py-2 bg-slate-100 border border-slate-300 rounded-[14px] text-[10px] font-black text-slate-800 uppercase tracking-wider">
                    {state.categories.find(c => c.id === e.categoryId)?.name}
                  </span>
                </td>
                <td className="p-8">
                  {e.isRestricted ? (
                    <span className="flex items-center gap-2 text-red-700 font-black text-[11px] uppercase bg-red-100 px-4 py-2 rounded-xl border border-red-200 shadow-sm"><ShieldAlert size={16}/> Restrição</span>
                  ) : (
                    <span className="text-emerald-700 font-black text-[11px] uppercase bg-emerald-100 px-4 py-2 rounded-xl border border-emerald-200 shadow-sm">Disponível</span>
                  )}
                </td>
              </tr>
            ))}
